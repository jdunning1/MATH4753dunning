...
mychisim<-function(n1=10,sigma1=3,mean1=5,iter=1000,ymax=0.1,...){    ## The beginning of the function, with inputs n1, sigma1, mean1, iter, and ymax
  y1=rnorm(n1*iter,mean=mean1,sd=sigma1)# generate iter samples of size n1

  data1.mat=matrix(y1,nrow=n1,ncol=iter,byrow=TRUE) ## Creates a matrix called data1.mat where each column is sample size n1

  ssq1=apply(data1.mat,2,var) ## Create ssq1 which is equal to S squared

  w=(n1-1)*ssq1/sigma1^2      ## This places the data into the chi-squared distribution

  hist(w,freq=FALSE, ylim=c(0,ymax), ## Creates a histogram with annotations
       main=substitute(paste("Sample size = ",n[1]," = ",n1," statistic = ",chi^2)),
       xlab=expression(paste(chi^2, "Statistic",sep=" ")), las=1)
  lines(density(w),col="Blue",lwd=3) ## Adds a density plot to the histogram
  curve(dchisq(x,n1-1),add=TRUE,col="Red",lty=2,lwd=3) ## Adds a theoretical curve to the histogram
  title=expression(chi^2==frac((n[1]-1)*s^2,sigma^2)) ## Allows mathematical notation to be written in the histogram title
  legend("topright",c("Simulated","Theoretical"),col=c("Blue","Red"),lwd=4,lty=1:2,bty="n",title=title) ## Generates the legend of the histogram
  return(list(w=w,summary=summary(w),sd=sd(w),fun="Chi-sq")) ## Returns a summary of the variables entered and the values found once plugged in
}
windows() ## Opens a new window
